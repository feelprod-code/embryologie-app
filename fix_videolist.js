...
// The first regex was not matching properly because of exact formatting. Using simpler replacements.
...
// Clean layer button
content = content.replace(/const isTouchedLayer = touchedLayerId === layer;\n\n\s*return \(\n\s*<button([\s\S]*?)onTouchStart=\{.*?\}\n\s*onTouchEnd=\{.*?\}\n\s*onTouchCancel=\{.*?\}\n\s*className=\{cn\([\s\S]*?\)\}\n\s*style=\{isSelected[\s\S]*?transform: isTouchedLayer \? 'scale\(0\.96\)' : 'scale\(1\)' \}\n\s*: \{ borderColor[\s\S]*?transform: isTouchedLayer \? 'scale\(0\.96\)' : 'scale\(1\)' \}\n\s*\}/g,
`return (
