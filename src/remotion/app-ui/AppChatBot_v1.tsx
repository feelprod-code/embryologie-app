...
export const AppChatBot: React.FC = () => {
    ...
    // 4. On scroll pour un effet marketing (moins rapide : 7 secondes)
    const scrollY = interpolate(frame, [230, 650], [0, -4621], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });

    // 5. Curseur de souris (apparaît et va vers le bouton, juste avant la fin du scroll)
    const cursorMove = spring({ frame: Math.max(0, frame - 640), fps, config: { damping: 14, stiffness: 120 } });
    
    // 6. Survol (Hover) = changement de couleur direct (dès que la souris est dessus)
    const isHovered = frame >= 655;

    // 7. Clic rapide
    const cursorClick = spring({ frame: Math.max(0, frame - 665), fps, config: { damping: 100, stiffness: 400 } });
    ...
    return (
        <div className="w-full h-full bg-[#FAF8F5] flex flex-col relative font-sans text-slate-800">
            ...
            {/* SOUND EFFECT */}
            <Sequence name="Bruitage - click.wav" from={665} durationInFrames={30}>
