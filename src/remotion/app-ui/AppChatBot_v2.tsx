...
import { useCurrentFrame, spring, useVideoConfig, interpolate, Sequence, Audio, staticFile } from 'remotion';
import { fpsS } from '../hooks/useTime';

export const AppChatBot: React.FC = () => {
    const actualFrame = useCurrentFrame();
    const { fps: realFps } = typeof useVideoConfig === 'function' && useVideoConfig() ? useVideoConfig() : { fps: 30 };
    const frame = actualFrame * (30 / realFps);
    const fps = 30;

    // 1. Apparition de la question utilisateur (f=40)
    const userMessageAnim = spring({ frame: Math.max(0, frame - 40), fps, config: { damping: 14 } });
    
    // 2. Le loader apparaît à f=60, et disparaît à f=100
    const loaderOpacity = interpolate(frame, [60, 70, 90, 100], [0, 1, 1, 0], { extrapolateRight: 'clamp', extrapolateLeft: 'clamp' });
    const loaderAnimY = spring({ frame: Math.max(0, frame - 60), fps, config: { damping: 14 } });
    
    // 3. La réponse finale apparaît à f=100
    const responseAnim = spring({ frame: Math.max(0, frame - 100), fps, config: { damping: 14 } });

    // 4. On scroll pour un effet marketing (rapide)
    const scrollY = interpolate(frame, [120, 240], [0, -4621], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });

    // 5. Curseur de souris (apparaît et va vers le bouton, juste avant la fin du scroll)
    const cursorMove = spring({ frame: Math.max(0, frame - 260), fps, config: { damping: 14, stiffness: 120 } });
    
    // 6. Survol (Hover)
    const isHovered = frame >= 265;

    // 7. Clic rapide
    const cursorClick = spring({ frame: Math.max(0, frame - 275), fps, config: { damping: 100, stiffness: 400 } });
    ...
    return (
        <div className="w-full h-full bg-[#FAF8F5] flex flex-col relative font-sans text-slate-800">
            ...
            {/* SOUND EFFECT */}
            <Sequence name="Bruitage - click.wav" from={fpsS(275, realFps)} durationInFrames={fpsS(30, realFps)}>
