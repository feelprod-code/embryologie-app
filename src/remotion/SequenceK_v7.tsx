...
export const SequenceK_v7: React.FC = () => {
    ...
    const text1Fly = spring({ frame: Math.max(0, frame - 5), fps, config: smoothSpring });
    const text2Fly = spring({ frame: Math.max(0, frame - 10), fps, config: smoothSpring });
    const text3Fly = spring({ frame: Math.max(0, frame - 15), fps, config: smoothSpring });
    const text4Fly = spring({ frame: Math.max(0, frame - 20), fps, config: smoothSpring });
    
    // --- ANIMATIONS D'EXPORT PDF (FIN DE SCÈNE) ---
    // 4. Sortie de l'iPhone et Entrée du Document PDF (frame 860, juste après le clic 835 et le chargement)
    // Au clic, on switche IMMÉDIATEMENT (stiffness élevé, sans rebond) !
    const switchAnim = spring({ frame: Math.max(0, frame - 860), fps, config: { damping: 20, stiffness: 220 } });
    ...
    // Démarrage frame 880, fin frame 1050
    const pdfScroll = interpolate(frame, [880, 1050], [0, -3400], { 
        ...
        <AbsoluteFill className="bg-[#FAF6ED] flex items-center justify-center font-sans overflow-hidden" style={{ perspective: '1500px' }}>
            ...
            <div 
                ...
                style={{
                    left: '50%',
                    top: '50%',
                    transform: `translate(calc(-50% + ${phoneKX}px), calc(-50% - 140px + ${phoneKYExit}px))`,
                    width: phoneW,
                    height: phoneH
                }}
            >
                ...
                <div className="relative w-full h-full shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] rounded-[45px]" style={{ transformStyle: 'preserve-3d' }}>
                    ...
                    {frame >= 20 && (() => {
                        const popAnim = spring({ frame: Math.max(0, frame - 20), fps, config: { damping: 100 } });
                        const pulse = interpolate((frame - 20) % 45, [0, 45], [0, 1]);
                        const magScale = spring({ frame: Math.max(0, frame - 27), fps, config: { damping: 100 } });
                        const floatY = Math.sin((frame - 27) * 0.05) * 5;
            ...
            {/* ---------------------------------------------------------------- */}
            {/* GROUPE FINAL CENTRÉ : PDF (5)                                    */}
            {/* ---------------------------------------------------------------- */}
            <div className="absolute inset-0 z-40 flex items-center justify-center pointer-events-none">
                
                {/* 5. AFFICHE LE FICHIER PDF A4 (Monte par le bas) */}
                <div 
                    className="w-[75%] rounded-xl overflow-hidden shadow-[0_40px_80px_-20px_rgba(0,0,0,0.6)] border border-slate-300 relative bg-[#FAF8F5] shrink-0 pointer-events-auto"
                    style={{ 
                        aspectRatio: '1/1.414', 
