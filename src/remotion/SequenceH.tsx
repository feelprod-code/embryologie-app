import { AbsoluteFill, useCurrentFrame, interpolate, spring, useVideoConfig, Img, Easing, staticFile } from 'remotion';
...
export const SequenceH: React.FC = () => {
    ...
    // 0 -> 30: Cursor flies from bottom to Previous Button
    // 30: Click "Previous Video"
    // 30 -> 60: Video Title & Duration changes. 
    ...
    const phoneRotateYPhase1 = interpolate(shiftProgress, [0, 1], [0, 8]); 
    const finalRotateY = interpolate(navTransition, [0, 1], [phoneRotateYPhase1, -25]); 
    const phoneRotateXPhase1 = 0;
    ...
    let audioProgress = shiftProgress > 0.5 ? 0.0 : 0.0675; // 0.0675 to match Sequence G Scrubber end frame 510
    let textProgress = shiftProgress > 0.5 ? 0.0 : 0.12656; // 0.12656 exactly matches Sequence G frame 510
    ...
    // Pointer cursor exactly over the button
    const click1 = 1 - (spring({ frame: Math.max(0, frame - 30), fps, config: { damping: 14 } }) * 0.2) + (spring({ frame: Math.max(0, frame - 40), fps, config: { damping: 14 } }) * 0.2);

    // Pointer cursor fades in at frame 21, clicks at 30, then fades out immediately
    let pointerOpacity = 0;
    if (frame >= 21 && frame <= 45) {
        pointerOpacity = interpolate(frame, [21, 23, 31, 38], [0, 1, 1, 0], { extrapolateRight: 'clamp', extrapolateLeft: 'clamp' });
    }

    if (frame < 60) {
        // Appears directly on Previous Video Button (ChevronLeft)
        cursorX = 30; 
        cursorY = 155;
    ...
    // Scrubber Time math 
    const currentSeconds = interpolate(audioProgress, [0, 1], [0, 519]);
    const m = Math.floor(currentSeconds / 60);
    const s = Math.floor(currentSeconds % 60).toString().padStart(2, '0');
    const currentScrubberTimeStr = shiftProgress > 0.5 ? `${m}:${s}` : "0:35";

    // Highlighter Helper for OEIL-04
    ...
    // Highlighter Helper for INFLUENCE-05 (Matches Sequence G EXACTLY)
    const Highlighter05 = ({ min, max, children }: { min: number, max: number, children: React.ReactNode }) => {
        ...
        <AbsoluteFill className="bg-[#FAF6ED] flex items-center justify-center overflow-hidden">
            ...
            >
                ...
                <p className="font-sans text-[26px] text-slate-500 font-medium leading-snug mt-5 border-l-4 border-slate-300 pl-6 max-w-xl">
                    Une toute nouvelle expéééérience...
                </p>
            </div>
            ...
            <div 
                ...
                style={{ 
                    left: '50%',
                    top: '50%',
                    transform: `translate(calc(-50% + ${interpolate(zoomToLeft, [0, 1], [currentX, (phoneW/2) - 580])}px), -50%) scale(${interpolate(zoomToLeft, [0, 1], [1, 1.7825])}) perspective(1200px) rotateY(${interpolate(zoomToLeft, [0, 1], [finalRotateY, 12])}deg) rotateX(${interpolate(zoomToLeft, [0, 1], [finalRotateX, 2])}deg) translateZ(0px)`,
                ...
                <div className="flex-1 overflow-hidden flex flex-col bg-[#FAF6ED] relative">
                        ...
                        <div className="px-4 py-3">
                            ...
                            <div className="flex justify-between items-center mb-4">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-slate-500 font-bold text-[13px]">{shiftProgress > 0.5 ? "08:39" : "03:50"}</span>
                                </div>
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <div className="flex-1 ml-4 flex items-center gap-2 text-[12px] text-slate-500 font-semibold font-sans">
                                    ...
                                    <span className="w-[30px] text-left tabular-nums">{shiftProgress > 0.5 ? "08:39" : "8:39"}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="px-4 py-3 bg-[#FAF6ED] shrink-0 border-b border-transparent relative z-20">
                        <div className="flex items-center relative gap-1 bg-white p-[5px] rounded-[12px] shadow-sm border border-slate-200/50">
                            <div className={`flex-1 flex justify-center py-2.5 rounded-[8px] text-[13px] transition-all duration-300 pointer-events-none font-medium text-slate-500 border border-transparent`}>
                                Résumé
                            </div>
                    ...
                    <div className="flex-1 px-5 pt-3 relative overflow-hidden bg-[#FAF6ED]">
                            ...
                            >
                                ...
                                <p className="text-[14px] leading-relaxed text-slate-500 font-normal pb-4 font-sans">
                                    <Highlighter05 min={0.0} max={0.16}>
                                    ...
                                    {" "}
                                    <Highlighter05 min={0.16} max={0.30}>
                                    ...
                                    {" "}
                                    <Highlighter05 min={0.30} max={0.60}>
                                ...
                                <p className="text-[14px] leading-relaxed text-slate-500 font-normal pb-10 font-sans">
                                    <Highlighter05 min={0.80} max={0.90}>Imaginez un axe central autour duquel se développe le tube neural.</Highlighter05>
                                    {" "}
                                    <Highlighter05 min={0.90} max={1.05}>Cet axe notochordal fournit des informations par un <b className={textProgress >= 0.90 ? 'text-[#1e293b]' : ''}>champ électromagnétique</b> de position,</Highlighter05>
                                    {" "}
                                    <Highlighter05 min={1.05} max={1.25}>agissant comme un <b className={textProgress >= 1.05 ? 'text-[#1e293b]' : ''}>GPS</b> pour les cellules environnantes, ce qui entraîne la réaction de diverses <b className={textProgress >= 1.05 ? 'text-[#1e293b]' : ''}>protéines</b>. Par exemple, les yeux se développeront à un endroit précis, tandis que d'autres</Highlighter05>
                                </p>
                            </div>
                        <div className="absolute bottom-0 left-0 right-0 h-[60px] bg-gradient-to-t from-[#FAF6ED] via-[#FAF6ED]/80 to-transparent pointer-events-none" />
                    </div>
