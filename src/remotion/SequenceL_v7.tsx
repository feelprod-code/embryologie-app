...
export const SequenceL_v7: React.FC = () => {
    const actualFrame = useCurrentFrame();
    const { fps: realFps } = useVideoConfig();
    const frame = actualFrame * (30 / realFps);
    const fps = 30;

    // --- PHASE 1 : TITRES PERCUTANTS (VERTICAL) ---
    const titles = [
        { 
            text: "L'ÉQUIVALENT DE 4 SÉMINAIRES", 
            className: "font-bebas text-[90px] text-[#1c2e4a] leading-[0.9] tracking-wide drop-shadow-sm text-center px-6"
        },
        { 
            text: "24 HEURES DE COURS EN VIDÉO", 
            className: "font-anton text-white bg-[#4171B5] text-[40px] tracking-widest uppercase px-6 py-3 rounded-2xl shadow-xl border-4 border-white mt-8 text-center"
        },
        { 
            text: "SOUS-TITRES EN 7 LANGUES", 
            className: "font-sans font-bold text-[35px] text-slate-500 italic border-l-[6px] border-[#4171B5] pl-6 mt-8 self-center text-center"
        },
        { 
            text: "RÉVISIONS CHRONOLOGIQUES", 
            className: "font-anton text-white bg-[#F27D33] text-[40px] tracking-wide uppercase px-6 py-3 rounded-[20px] shadow-[0_15px_30px_rgba(242,125,51,0.3)] border-4 border-white rotate-[-1.5deg] mt-8 text-center"
        },
        { 
            text: "ASSISTANT I.A. INTÉGRÉ", 
            className: "font-anton text-[#5A9C51] text-[60px] tracking-widest uppercase mt-8 drop-shadow-sm text-center"
        },
        { 
            text: "EXPORT PDF & RÉFÉRENCES", 
            className: "px-6 py-4 rounded-full border-2 border-slate-300 bg-white/70 text-[#1c2e4a] font-bold tracking-[0.2em] text-[28px] shadow-sm backdrop-blur-md mt-8 text-center"
        }
    ];
    ...
    // Texte "Disponible sur tous vos écrans" centré en HAUT
    ...
    return (
        <AbsoluteFill className="bg-[#FAF6ED] overflow-hidden flex items-center justify-center font-sans">
            ...
            {frame < mockupsStart + 20 && (
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-0 px-4 w-full">
                    {titles.map((title, i) => {
            ...
            {/* --- PHASE 2 : MOCKUPS & ÉCRANS (VERTICAL) --- */}
            {frame >= mockupsStart && (
                <AbsoluteFill className="z-10 flex flex-col items-center justify-center">
                    
                    {/* Titre Principal (Haut de l'écran) */}
                    <div 
                        className="absolute top-16 left-0 right-0 flex justify-center text-slate-800 font-medium text-[45px] tracking-tight z-40"
                        style={{
                            opacity: finalPhraseOpacity,
                            transform: `translateY(${interpolate(finalPhraseY, [0, 1], [-40, 0])}px)`
                        }}
                    >
                    ...
                    <div className="relative w-full h-full flex flex-col items-center justify-center perspective-[2500px]">
                        ...
                        <div 
                            ...
                            style={{
                                transform: `
                                    translateX(0px)
                                    translateY(${interpolate(imacEntry, [0, 1], [-500, -150])}px) 
                                    translateZ(-150px)
                                    scale(${interpolate(imacEntry, [0, 1], [0.4, 0.5])})
                                `,
                                opacity: imacEntry,
                        ...
                        {/* ======================= 2. iPAD (Portrait) - MIDDLE ======================= */}
                        <div 
                            className="absolute z-20"
                            style={{
                                transform: `
                                    translateX(0px)
                                    translateY(${interpolate(ipadEntry, [0, 1], [-200, 150])}px) 
                                    translateZ(50px)
                                    scale(${interpolate(ipadEntry, [0, 1], [0.6, 0.7])})
                                `,
                                opacity: ipadEntry,
                            }}
                        >
                        ...
                        {/* ======================= 3. iPHONE (Mobile) - BOTTOM ======================= */}
                        <div 
                            className="absolute z-30"
                            style={{
                                transform: `
                                    translateX(0px)
                                    translateY(${interpolate(iphoneEntry, [0, 1], [200, 500])}px)
                                    translateZ(250px)
                                    scale(${interpolate(iphoneEntry, [0, 1], [0.7, 0.85])})
                                `,
                                opacity: iphoneEntry,
                            }}
                        >
