...
export const VideoPlayerPage: React.FC<VideoPlayerPageProps> = ({ course: initialCourse, onSelectVideo }) => {
  ...
  const handleTimeUpdate = (currentTime: number) => {
    ...
    const debugEl = document.getElementById('vtt-debug-info-v2');
    if (debugEl) {
      debugEl.innerHTML = `iOS TEST V2<br/>Cues: ${cuesRef.current.length}<br/>Time: ${currentTime.toFixed(1)}s<br/>Active: ${active ? 'YES' : 'NO'}`;
    }
  };
  return (
    <>
      {/* GLOBAL DEBUG OVERLAY V2 */}
      <div id="vtt-debug-info-v2" className="fixed top-2 left-2 z-[999999] bg-pink-600 text-white text-lg font-bold p-4 rounded-xl shadow-2xl border-4 border-white pointer-events-none">
        AWAITING V2 DATA...
      </div>
