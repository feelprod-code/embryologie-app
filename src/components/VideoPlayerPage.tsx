...
export const VideoPlayerPage: React.FC<VideoPlayerPageProps> = ({ course: initialCourse, onSelectVideo, hasFullAccess = false, onLockedVideoClick }) => {
  ...
  const currentPdfUrl = (pdfFileMapping as Record<string, string>)[course.id] || course.pdfUrl || '';
  ...
  const handleOfflineCache = async () => {
    ...
    setIsCaching(true);
    try {
      // On passe par le proxy Vite / Vercel pour éviter l'erreur CORS sur le téléchargement de Cloudflare
      const fetchUrl = videoUrl.replace('https://customer-6i2z59dst7q6iswv.cloudflarestream.com', '/cf-stream');
            ...
            <div className="flex items-center justify-between gap-1 sm:gap-2 max-w-3xl mx-auto w-full">
              ...
              <div className="flex flex-1 items-center justify-end gap-1.5 sm:gap-2 z-10">
                <button
                  ...
                  className="flex items-center gap-1 py-1 sm:py-1 md:py-1.5 px-2 sm:px-2.5 rounded-md sm:rounded-lg shadow-sm border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 text-[10px] sm:text-xs font-bold tracking-wider transition-all active:scale-95 shrink-0"
                  title="Consulter et feuilleter la fiche PDF dans le lecteur"
                >
    ...
    <div className={cn(
      ...
      <div className="flex-1 flex flex-col h-full w-full relative min-h-0 overflow-hidden">
        ...
        <div className={cn(
          ...
          "prose-img:rounded-xl prose-img:shadow-md prose-img:border prose-img:border-slate-200/60 prose-img:w-full prose-img:h-auto prose-img:my-8",
          "[&>*:first-child]:!mt-0"
        )}>
