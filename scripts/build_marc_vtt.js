...
const text = `[00:00:01:54 - 00:00:12:07]
...
for(let i=0; i<lines.length; i++) {
  ...
  if(line.startsWith('[')) {
     const regex = /\[(\d{2}:\d{2}:\d{2}):(\d{2})\s*-\s*(\d{2}:\d{2}:\d{2}):(\d{2})\]/;
     const match = line.match(regex);
     if(match) {
        const start = `${match[1]}.${match[2]}0`;
        const end = `${match[3]}.${match[4]}0`;
        vtt += `${start} --> ${end}\n`;
...
const outputPath = path.join(__dirname, '../public/vtt/8f890e7f51588216db016b73a8a97a14_fr.vtt');
fs.writeFileSync(outputPath, vtt);
console.log('Created VTT: ' + outputPath);
