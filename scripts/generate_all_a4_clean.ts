...
const categories: Record<string, { label: string; color: string; bg: string; masterFilename: string }> = {
    ectoderme: {
        label: "L'ECTODERME",
        color: "#5A9C51",
...
function buildLessonHtml(course: any): string {
...
function wrapFullDocument(contentHtml: string, catColor = '#5A9C51', catBg = 'rgba(90,156,81,0.08)'): string {
    return `
    <!DOCTYPE html>
    <html lang="fr">
    <head>
...
async function main() {
    ...
    for (const [catKey, catInfo] of Object.entries(categories)) {
        ...
        if (!fs.existsSync(catSingleDir)) {
            fs.mkdirSync(catSingleDir, { recursive: true });
        }

        // Clean out directory
        const existing = fs.readdirSync(catSingleDir);
        for (const f of existing) {
            fs.unlinkSync(path.join(catSingleDir, f));
        }
        ...
        for (let i = 0; i < catLessons.length; i++) {
            ...
            const relUrl = `/pdfs/${catKey}/${singlePdfName}`;
            mapping[lesson.id] = relUrl;
        ...
        if (masterCourse) {
            mapping[masterCourse.id] = `/pdfs/cours_complets/${catInfo.masterFilename}`;
        }
    }
